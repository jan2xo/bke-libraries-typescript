...
function isPackageSource(sourceFile: string) {
  ...
  return path === "module.manifest.ts" || path === "prisma.config.ts" || path.startsWith("contracts/") || path.startsWith("logic/") || path.startsWith("providers/") || path.startsWith("prisma/") || path.startsWith("tests/");
}
...
for (const path of ["module.manifest.ts", "prisma.config.ts", "prisma/schema.prisma", "migrations/0001_licensing_baseline/migration.sql", "migrations/0003_transfer_policy_facts/migration.sql", "contracts", "logic", "providers", "prisma/repositories", "tests", "docs"]) {
...
for (const sourceFile of packageSources) {
  ...
  for (const specifier of importsFrom(source)) {
    if (specifier.startsWith(".")) {
      ...
      if (!isWithin(moduleRoot, target)) { violations.push(`${sourcePath} escapes Licensing through ${specifier}`); continue; }
...
if (existsSync(schemaPath)) {
  ...
  for (const model of models) if (!allowedModels.has(model)) violations.push(`Licensing Prisma owns foreign model ${model}`);
  for (const expected of allowedModels) if (!models.includes(expected)) violations.push(`Licensing Prisma is missing owned model ${expected}`);
}
...
console.log(`Licensing extraction boundary GREEN: ${packageSources.length} package-owned source files; host composition excluded`);
