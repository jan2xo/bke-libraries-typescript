...
import type { CommerceSubscriptionLifecycleRepository } from "../../logic/subscription-lifecycle-management";
  ...
  return Object.freeze({
    ...
    async renew(input: RenewInput) {
      ...
      try {
        ...
        const plan = planCommerceSubscriptionRenewal({
          currentPeriodEnd: existing.currentPeriodEnd,
          intervalUnit: input.intervalUnit,
          intervalCount: input.intervalCount,
          now: input.now,
          settlementOfferId: input.settlementOfferId,
          discountedCyclesTotal: existing.discountedCyclesTotal,
          discountedCyclesConsumed: existing.discountedCyclesConsumed,
        });
        if (plan.status !== "PLANNED") throw new Error("Invalid Commerce subscription renewal facts.");
        ...
        const result = await client.query<SubscriptionRow>(
          ...
          [
            ...
            plan.renewalReminderAt,
            plan.nextDiscountedCyclesConsumed,
          ],
        );
        await client.query("COMMIT");
        return {
          subscription: mapSubscription(result.rows[0]!),
          discountedCycleConsumed: plan.discountedCycleConsumed,
        };
      } catch (error) {
        await client.query("ROLLBACK");
        throw error;
