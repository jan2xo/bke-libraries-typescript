...
import {
  ...
  planCommerceSubscriptionRenewal,
...
describe("Commerce subscription lifecycle management", () => {
  ...
  it("plans renewal from the later of current period end and now", () => {
    expect(planCommerceSubscriptionRenewal({
      ...
      currentPeriodStart: new Date("2026-02-15T12:00:00.000Z"),
      currentPeriodEnd: new Date("2026-03-15T12:00:00.000Z"),
      renewalReminderAt: new Date("2026-03-08T12:00:00.000Z"),
    ...
    expect(planCommerceSubscriptionRenewal({
      ...
      currentPeriodStart: new Date("2026-04-10T09:30:00.000Z"),
      currentPeriodEnd: new Date("2027-04-10T09:30:00.000Z"),
      renewalReminderAt: new Date("2027-03-11T09:30:00.000Z"),
      discountedCycleConsumed: true,
      nextDiscountedCyclesConsumed: 3,
    });
  });

  it("does not consume promotional cycles without a settlement offer or after exhaustion", () => {
    expect(planCommerceSubscriptionRenewal({
      currentPeriodEnd: new Date("2026-02-15T12:00:00.000Z"),
      intervalUnit: "MONTH",
      intervalCount: 1,
      ...
      discountedCyclesTotal: 3,
      discountedCyclesConsumed: 1,
    })?.discountedCycleConsumed).toBe(false);
    ...
    expect(planCommerceSubscriptionRenewal({
      ...
      settlementOfferId: "offer-1",
      discountedCyclesTotal: 3,
      discountedCyclesConsumed: 3,
    })).toMatchObject({ discountedCycleConsumed: false, nextDiscountedCyclesConsumed: 3 });
  });

  it("fails closed when renewal facts are invalid", () => {
    expect(planCommerceSubscriptionRenewal({
