    ...
    plan(input: CommercePaymentOutcomeInput): CommercePaymentOutcomePlan {
      if (input.kind === "PAYMENT_PAID") {
        ...
        return {
          ...
          auditAction: input.orderStatus === "CANCELLED"
      ...
      if (input.refundStatus !== "succeeded") {
        return {
          ...
          refundOperationErrorCode: input.refundStatus === "failed" ? "PAYMENT_REFUND_NOT_ALLOWED" : null,
        };
      }

      if (input.orderStatus === "REFUNDED") return { status: "NOOP", reason: "ALREADY_REFUNDED" };
      if (input.orderStatus !== "PAID") return { status: "REJECTED", code: "PAYMENT_REFUND_CONFLICT" };
