import type {
  ...
  PaymentsProviderEventModeInput,
  PaymentsProviderEventModeResult,
...
export function validatePaymentsProviderEventMode(
  input: PaymentsProviderEventModeInput,
): PaymentsProviderEventModeResult {
  return input.eventLivemode === input.expectedLivemode
    ? { status: "MATCHED" }
    : { status: "REJECTED", code: "PAYMENT_MODE_MISMATCH" };
}

export function matchPaymentsCommercialEvent(
  input: PaymentsCommercialEventMatchInput,
): PaymentsCommercialEventMatchResult {
  const mode = validatePaymentsProviderEventMode({
    eventLivemode: input.event.livemode,
    expectedLivemode: input.expectedLivemode,
  });
  if (mode.status === "REJECTED") return mode;
...
export function createPaymentsCommercialEventMatchCapability(): PaymentsCommercialEventMatchCapability {
  return Object.freeze({
    validateMode: validatePaymentsProviderEventMode,
    match: matchPaymentsCommercialEvent,
  });
}
