...
import type { IdentityPasswordAuthenticationRecord, IdentityRepository } from "../logic/identity-repository";
...
function verifier(verify: IdentityPasswordVerifier["verify"] = async () => true): IdentityPasswordVerifier { return { verify }; }
...
describe("Identity password authentication capability", () => {
  ...
  it("collapses missing identities and bad passwords into INVALID_CREDENTIALS", async () => {
    ...
    const wrongPassword = createIdentityPasswordAuthenticationCapability(repository(), verifier(async () => false));
    await expect(missing.authenticate({ email: "missing@example.com", password: "password" })).resolves.toEqual({ status: "INVALID_CREDENTIALS" });
    await expect(wrongPassword.authenticate({ email: "customer@example.com", password: "wrong" })).resolves.toEqual({ status: "INVALID_CREDENTIALS" });
  });
  it("rejects invalid input without touching persistence", async () => {
    let called = false;
    const identity = createIdentityPasswordAuthenticationCapability(repository({ findPasswordAuthenticationByEmail: async () => { called = true; return authenticationRecord(customer); } }), verifier());
    await expect(identity.authenticate({ email: "not-an-email", password: "password" })).resolves.toEqual({ status: "FAILED", code: "INVALID_INPUT" });
    await expect(identity.authenticate({ email: "customer@example.com", password: "" })).resolves.toEqual({ status: "FAILED", code: "INVALID_INPUT" });
    expect(called).toBe(false);
  });
  it("maps repository failure to PERSISTENCE_UNAVAILABLE", async () => {
    const identity = createIdentityPasswordAuthenticationCapability(repository({ findPasswordAuthenticationByEmail: async () => { throw new Error("database unavailable"); } }), verifier());
    await expect(identity.authenticate({ email: "customer@example.com", password: "password" })).resolves.toEqual({ status: "FAILED", code: "PERSISTENCE_UNAVAILABLE" });
  });
  it("fails closed when password verification cannot verify the stored hash", async () => {
    const identity = createIdentityPasswordAuthenticationCapability(repository(), verifier(async () => { throw new Error("unverifiable hash"); }));
    await expect(identity.authenticate({ email: "customer@example.com", password: "password" })).resolves.toEqual({ status: "INVALID_CREDENTIALS" });
  });
});
